...
export default function AdvancedFilterDemo() {
    ...
    const [taskData, setTaskData] = useState({ id: "", name: "", leave_type: "", date: "", end_date: "", reason: "", status: "" });
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch(`http://localhost:5000/api/leaves/${taskData.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    return (
        ...
        <button className="btn btn-primary" onClick={() => {setTaskData({  id: "", name: "", leave_type: "", date: "", end_date: "", reason: "", status: "" }); openCreateModal()}}>
    Create New Leave
</button>
            <DataTable value={customers} paginator showGridlines rows={10} loading={loading} dataKey="id"
                ...
                <Column header="Leave type" filterField="leave_type" style={{ minWidth: '12rem' }} body={leave_typeBodyTemplate}
                ...
                <Column header="Start Date" filterField="date" dataType="date" style={{ minWidth: '10rem' }} body={dateBodyTemplate} filter filterElement={dateFilterTemplate} />
                ...
                <Column field="reason" header="Reason" filter filterPlaceholder="Search by name" style={{ minWidth: '12rem' }} />
    ...
    <div className="mb-3">
        ...
        <textarea
            ...
            onChange={(e) => setTaskData({ ...taskData, reason: e.target.value })}
            required
        ></textarea>
    </div>

    <div className="mb-3">
        <label className="form-label">Leaves Type</label>
        <input
            type="text"
            className="form-control"
            value={taskData?.leave_type || ""}
            onChange={(e) => setTaskData({ ...taskData, leave_type: e.target.value })}
            required
        />
    </div>
